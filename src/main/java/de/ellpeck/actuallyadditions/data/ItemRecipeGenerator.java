...
/*        Recipe.shaped(ActuallyItems.RING_OF_GROWTH.get())
	...
	private static void dyeDrill(RegistryObject<? extends Item> result, TagKey<Item> dyeItem, Consumer<FinishedRecipe> consumer) {
		Recipe.shapeless(result.get())
			.requires(TargetNBTIngredient.of(ActuallyTags.Items.DRILLS))
			.requires(dyeItem)
			.save(
				new RecipeInjector<ShapelessRecipeBuilder.Result>(consumer, RecipeKeepDataShapeless.Result::new),
				new ResourceLocation(ActuallyAdditions.MODID, "drill_coloring/dye_" + BuiltInRegistries.ITEM.getKey(result.get()).getPath())
			);
	}
